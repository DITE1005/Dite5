...
@property (nonatomic,strong)UIButton *tanmuButton;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.bottomBar addSubview:self.tanmuButton];
...
- (void)layoutSubviews
{
    ...
    self.indicatorView.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds));
     ...
     self.tanmuButton.frame = CGRectMake(CGRectGetMaxX(self.progressSlider.bounds), CGRectGetMaxY(self.bottomBar.bounds)/2 - CGRectGetMaxY(self.progressSlider.bounds)/2, CGRectGetMaxX(self.bounds) - CGRectGetMaxX(self.playButton.bounds) - CGRectGetMaxX(self.fullScreenButton.bounds), CGRectGetMaxY(self.progressSlider.bounds));
...
-(UIButton *)tanmuButton{
    if (!_tanmuButton) {
        _tanmuButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_tanmuButton setImage:[UIImage imageNamed:[self videoImageName:@"mine-sun-icon-click"]] forState:UIControlStateNormal];
    }
    return _tanmuButton;
