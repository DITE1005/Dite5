//
//  LatestBeautyTableViewController.m
//  Product-B
//
...
@interface LatestBeautyTableViewController ()
...
@implementation LatestBeautyTableViewController
...
- (void)viewDidLoad {
    ...
    self.limitIndex = 1;
    self.Url =[NSString stringWithFormat:@"http://s.budejie.com/topic/tag-topic/117/new/bs0315-iphone-4.3/0-%ld.json",self.limitIndex * 40];
    ...
    tableView.mj_footer = [XSFooterRefresh footerWithRefreshingBlock:^{
        // 模拟延迟加载数据，因此2秒后才调用（真实开发中，可以移除这段gcd代码）
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.Url =[NSString stringWithFormat:@"http://s.budejie.com/topic/tag-topic/117/new/bs0315-iphone-4.3/0-%ld.json",(self.limitIndex + 1) * 40];
            self.limitIndex += 1;
            [self.topicFrame removeAllObjects];
            // 请求数据
            [self requestData];
